...
use crate::live_insertion::{SessionLookaheadOverride, SessionStartOverrides};
...
impl TranscriptionCoordinator {
    ...
    pub fn send_external_input(&self, binding_id: &str, source: &str) {
        self.send_external_input_with_overrides(
            binding_id,
            source,
            SessionStartOverrides::default(),
        );
    ...
    pub fn send_external_input_with_overrides(
        &self,
        binding_id: &str,
        source: &str,
        overrides: SessionStartOverrides,
...
fn run_effect(app: &AppHandle, state: &mut CoordinatorState, effect: Effect) {
    match effect {
        Effect::Start {
            ...
            let started = start(app, &binding_id, &hotkey_string, overrides);
...
fn start(
    ...
    if let Some(slot) = app.try_state::<Arc<SessionLookaheadOverride>>() {
        slot.store(overrides);
    }
...
mod tests {
    ...
    fn ptt_input(is_pressed: bool) -> InputEvent {
        InputEvent {
            ...
            external: false,
            overrides: SessionStartOverrides::default(),
        }
    }
    ...
    #[test]
    fn drain_preserves_session_start_overrides() {
        let mut state = CoordinatorState::new();
        let now = Instant::now();
        drive_into_processing(&mut state, now);

        let expected = SessionStartOverrides {
            lookahead: None,
            live_insertion: true,
        };
        let mut pending = toggle_input(true);
        pending.overrides = expected;
        assert!(state
            .on_input(pending, now + Duration::from_millis(200))
            .is_none());

        match state.on_processing_finished() {
            Some(Effect::Start { overrides, .. }) => assert_eq!(overrides, expected),
            other => panic!("expected deferred start, got {other:?}"),
        }
    }
